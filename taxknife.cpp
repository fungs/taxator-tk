...
int main( int argc, char** argv ) {
	...
	vector< string > rank_names, ranks, pmode;
	...
	( "mode", po::value< vector <string> >( &pmode),"choose mode:\n"
                                          "\"traverse\":\ttraverse nodes up to a given rank\n\n"
                                          "\"names\":\tlooks up the name or rankname of a node\n\n")
	( "field,f", po::value< unsigned int >( &field_pos )->default_value( 1 ), "input column number to use\n" );

	po::options_description rankf("Rank-filter");
	rankf.add_options()
    ...
    po::options_description namef("Name-filter");
    namef.add_options()
    ...
    if(find(pmode.begin(), pmode.end(),"traverse") != pmode.end()){
        // command line arguments

        if( ! vm.count( "traverse-ranks" ) ) {
            cout << "Required argument \"--traverse-ranks\" is not given.\n" << desc << endl;
            return EXIT_FAILURE;
        }
    ...
    else if(find(pmode.begin(), pmode.end(),"names") != pmode.end()){

        if( ! vm.count( "names-ranks" ) ) ranks = default_ranks;
