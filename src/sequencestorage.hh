...
public:
    RandomInmemorySeqStoreRO ( const std::string& filename ) : format_( Format() ) {
        ...
        seqan::StringSet<WorkingStringType> seqs;

        seqan::readRecords(ids, seqs, db_sequences);
        large_unsigned_int num_records = seqan::length( ids );
        
        std::cerr  << "' (total=" << num_records << ")" << std::endl;
        boost::progress_display eta( num_records - 1, std::cerr ); //progress bar
        
        auto id = seqan::begin(ids);
        auto seq = seqan::begin(seqs);
...
//        } else BOOST_THROW_EXCEPTION(FileError{} << file_info{filename});
    ...
    //const StorageStringType& getSequence ( const seqan::CharString& id ) const {
...
//             std::cerr << "Index \"" << index_filename << "\" for \"" << fasta_filename << "\" not found, building..." << std::endl;
        ...
        // is removing .refNameStore right ?
        for (auto it = seqan::begin(index_.seqNameStore); !seqan::atEnd(it); seqan::goNext(it)) {
            //unsigned itval;
            //std::cerr << *it << "\n";
            //seqan::lexicalCast(itval,*it);
            //refid2position_[seqan::sequenceName(index_,itval)] = idx++;
