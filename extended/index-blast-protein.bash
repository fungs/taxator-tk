...
ref_fasta="$ref_root"/refdata.faa
...
mkdir -p "$cpath"
cd "$cpath"
$time_cmd -p -o "$dbname".makedb.time makeblastdb -in "$ref_fasta" -dbtype prot -input_type fasta -logfile "$dbname".makedb.log -out "$dbname" 2>/dev/null && echo 'Success.' || echo 'Failed.' 1>&2
